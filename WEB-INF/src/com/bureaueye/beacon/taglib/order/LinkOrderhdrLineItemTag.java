...
public class LinkOrderhdrLineItemTag extends TagSupport {
     ...
     * The attribute name.
     */
    private String name = "orderhdrLineItem";
	...
	url.append(config.getPrefix());
        url.append(page);
        OrderhdrDTO lineItem = null;
	try {
		lineItem = (OrderhdrDTO) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noOrderhdr", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getId().toString()));	
