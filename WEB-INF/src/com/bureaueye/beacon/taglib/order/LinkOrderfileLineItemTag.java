...
public class LinkOrderfileLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "orderfileLineItem";

		Orderfile orderfile = null;
		try {
			orderfile = (Orderfile) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (orderfile == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOrderfile", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				orderfile.getOrderhdrId().toString()));
		url.append("&orderfileId=");
		url.append(TagUtils.getInstance().filter(
				orderfile.getOrderfileId().toString()));
