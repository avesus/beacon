...
public class LinkCostLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		Cost cost = null;
		String name = "costLineItem";

		try {
			cost = (Cost) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (cost == null)
			throw new JspException(messages.getMessage("lineItem.noCost",
					name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("costkey=");
		url.append(TagUtils.getInstance().filter(cost.getCostkey()));
