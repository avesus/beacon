...
public class LinkQuomovLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "quomovLineItem";

		QuomovDTO quomov = null;
		try {
			quomov = (QuomovDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (quomov == null)
			throw new JspException(messages.getMessage(
					"lineItem.noQuomov", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
