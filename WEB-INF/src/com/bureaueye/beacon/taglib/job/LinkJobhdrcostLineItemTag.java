...
public class LinkJobhdrcostLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "jobhdrcostLineItem";

		JobcostDTO jobcost = null;
		try {
			jobcost = (JobcostDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (jobcost == null)
			throw new JspException(messages.getMessage(
					"lineItem.noJobcost", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				jobcost.getJobhdrId().toString()));
		url.append("&jobcostId=");
		url.append(TagUtils.getInstance().filter(
				jobcost.getJobcostId().toString()));
