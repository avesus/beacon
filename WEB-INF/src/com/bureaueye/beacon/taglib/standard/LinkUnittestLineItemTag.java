...
public class LinkUnittestLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "unittestLineItem";

		Unittest unittest = null;
		try {
			unittest = (Unittest) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (unittest == null)
			throw new JspException(messages.getMessage(
					"lineItem.noUnittest", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				unittest.getUnitId().toString()));
		url.append("&unittestId=");
		url.append(TagUtils.getInstance().filter(
				unittest.getUnittestId().toString()));
