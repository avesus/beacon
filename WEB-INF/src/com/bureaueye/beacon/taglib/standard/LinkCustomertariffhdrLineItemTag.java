...
public class LinkCustomertariffhdrLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		CustomertariffhdrDTO customertariffhdr = null;
		String name = "customertariffhdrLineItem";

		try {
			customertariffhdr = (CustomertariffhdrDTO) pageContext
					.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (customertariffhdr == null)
			throw new JspException(messages.getMessage(
					"lineItem.noCustomertariffhdr", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append(page.indexOf("action=") < 0 ? "id=" : "custtarfhdrId=");
		url.append(customertariffhdr.getCusttarfhdrId());
