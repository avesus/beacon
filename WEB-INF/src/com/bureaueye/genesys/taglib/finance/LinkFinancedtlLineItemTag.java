...
public class LinkFinancedtlLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		FinancedtlDTO dto = null;
		try {
			dto = (FinancedtlDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (dto == null)
			throw new JspException(messages.getMessage(
					"lineItem.noFinancedtl", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("financehdrId=");
		url.append(TagUtils.getInstance().filter(
				dto.getFinancehdrId().toString()));
		url.append("&financedtlId=");
		url.append(TagUtils.getInstance().filter(
				dto.getFinancedtlId().toString()));
