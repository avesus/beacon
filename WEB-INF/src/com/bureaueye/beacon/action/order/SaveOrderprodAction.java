...
public final class SaveOrderprodAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		OrderprodForm orderprodForm = (OrderprodForm) form;
		String action = orderprodForm.getAction();
		...
		OrderprodBD bd = new OrderprodBD(this.getSessionFactoryClusterMap());

		Integer pk = null;	
		try {
			pk = new Integer(orderprodForm.getOrderprodId());			
		} catch (Exception e) {}
		Orderprod dto = bd.read(pk);
		if (dto == null) {
			dto = new Orderprod();
		}		
		...
		BeanUtils.copyProperties(dto, orderprodForm);

		
		if (action.equals("Delete")) {		
			bd.delete(new Orderprod(pk), user);		
		} else {
