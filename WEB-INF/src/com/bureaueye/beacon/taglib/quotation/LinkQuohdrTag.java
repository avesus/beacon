...
public class LinkQuohdrTag extends TagSupport {
     ...
     * The attribute name.
     */
    private String name = "QuohdrForm";
	...
	url.append(config.getPrefix());
        ...
        QuohdrForm form = null;
        try {
        	form = (QuohdrForm) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	form = null;
        }     
	if (form == null)
	    throw new JspException
	        (messages.getMessage("form.noQuohdr", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
