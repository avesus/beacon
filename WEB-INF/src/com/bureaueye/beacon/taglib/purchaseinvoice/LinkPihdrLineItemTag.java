...
public class LinkPihdrLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		PihdrDTO lineItem = null;
		try {
			lineItem = (PihdrDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
			lineItem = null;
		}
		if (lineItem == null)
			throw new JspException
			(messages.getMessage("lineItem.noPihdrLineItem", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("id=");
		url.append(TagUtils.getInstance().filter(lineItem.getPihdrId().toString()));	
