...
public class LinkIncotermLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			dao = (Incoterm) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (dao == null)
			throw new JspException(messages.getMessage("lineItem.noDAO",
					name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("incotermkey=");
		url.append(TagUtils.getInstance().filter(dao.getIncotermkey()));
