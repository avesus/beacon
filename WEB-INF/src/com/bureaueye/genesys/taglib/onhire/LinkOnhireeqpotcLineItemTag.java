...
public class LinkOnhireeqpotcLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		OnhireeqpotcDTO lineItem = null;
		try {
			lineItem = (OnhireeqpotcDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOnhireeqpotc", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append("&onhireeqpotcId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getOnhireeqpotcId().toString()));
