...
import com.bureaueye.beacon.action.order.pdf.EndPageBookingnotification;
...
/**
 */
public class SaveBookingnotificationAction extends BaseAction {
	...
	public SaveBookingnotificationAction() {
		try {
	...
	throws Exception {
		...
		if (saveForm.getBtnSubmit().equals("Print") || saveForm.getBtnSubmit().equals("Email")) {
			...
			_Document.open();				
			_PdfWriter.setPageEvent(new EndPageBookingnotification(saveForm.getCompanyheader()));
	...
	public void toPrint(PrinthdrForm saveForm, User user) throws ServletException, IOException
	{
		...
		try {
			...
			_Table1_1.addCell(new Phrase("Booking Notification",fontHeading));
			...
			// references label
			float[] widths1_9 = {0.15f, 0.2f, 0.2f, 0.45f};
			...
			try {
				...
				_Table1_9.addCell(new Phrase("Shipper Ref:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getShipperRef(),font));
				_Table1_9.addCell(new Phrase("Consignee:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getConsignee(),font));
				_Table1_9.addCell(new Phrase("Consignee Ref:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getConsigneeRef(),font));		
				_Table1_9.addCell(new Phrase("Customer:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getCustomer(),font));
				_Table1_9.addCell(new Phrase("Customer Ref:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getCustomerRef(),font));
				_Table1_9.addCell(new Phrase("Pickup at:",fontBold));
				_Table1_9.addCell(new Phrase(saveForm.getPickupPlant(),font));
			...
			// details header
			float[] widths1_2 = {0.20f, 0.2f, 0.4f, 0.2f};
			...
			_Table1_2.addCell(new Phrase("Product Information",fontSmallBold));
			_Table1_2.addCell(new Phrase("Product Weight",fontSmallBold));
			...
			for (Iterator it = saveForm.getPrintdtls().iterator(); it.hasNext();) {
				...
				try {
					// details header
					float[] widths1_4 = {0.20f, 0.2f, 0.4f, 0.2f};
