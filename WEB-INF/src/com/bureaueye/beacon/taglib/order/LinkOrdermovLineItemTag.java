...
public class LinkOrdermovLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "ordermovLineItem";

		OrdermovDTO ordermov = null;
		try {
			ordermov = (OrdermovDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (ordermov == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOrdermov", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				ordermov.getOrderhdrId().toString()));
		url.append("&ordermovId=");
		url.append(TagUtils.getInstance().filter(
				ordermov.getOrdermovId().toString()));
