...
public class LinkCityLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		City city = null;
		String name = "cityLineItem";

		try {
			city = (City) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (city == null)
			throw new JspException(messages.getMessage("lineItem.noCity", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("city=");
		url.append(TagUtils.getInstance().filter(city.getCity()));
