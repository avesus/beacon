...
public final class SavePidtlcostAction extends BaseAction {
	...
	throws Exception {
		...
		if (action.equals(com.bureaueye.beacon.bean.Constants.CREATE_ACTION)) {				
			...
			// THIS WORKS -- check ALL actual costs entered FOR PIDTL are not more than detail invoice amount			
			try {
				Pidtl dtldao = pidtlbd.read(new Integer(saveForm.getPidtlId()));
				
				float dtlinvoiceamt = pidtlbd.read(dtldao.getPidtlId()).getInvoiceamt().floatValue();
				float cstamtalloc = new BigDecimal(saveForm.getCstamtalloc()).floatValue();
				//retrieve other costs allocated to PI detail
				BigDecimal cstamtallocTotal=bd.pidtlcostsCstamtallocTotal(saveForm.getPidtlId());
				float allocatetotal = 0;
				if (cstamtallocTotal!=null) allocatetotal=cstamtallocTotal.floatValue()+cstamtalloc;
				else allocatetotal=cstamtalloc;
				
				log.debug(
						"["+this.getClass().getName()+"] "+
						new java.util.Date()+
						" detail invoice amount: " +dtlinvoiceamt+
						" allocate total: " +allocatetotal
						);
				
				if (allocatetotal > dtlinvoiceamt) {
					errors.add("informationMessage",new ActionMessage("errors.cannotbegreaterthan","Allocated Total","Invoice Amount"));					
				}		
