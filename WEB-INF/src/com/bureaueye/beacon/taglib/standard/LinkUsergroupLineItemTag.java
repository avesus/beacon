...
public class LinkUsergroupLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		Usergroup lineItem = null;
		String name = "usergroupLineItem";

		try {
			lineItem = (Usergroup) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage("lineItem.noUsergroup",
					name));
		...
		if (page.indexOf("?") < 0)
			...
			url.append("&");
		url.append(page.indexOf("action=") < 0 ? "id=" : "usergroupid=").append(
				TagUtils.getInstance().filter(lineItem.getUsergroupid()));
