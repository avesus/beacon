...
public class LinkOrdermovcostLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "ordermovcostLineItem";

		OrdercostDTO ordercost = null;
		try {
			ordercost = (OrdercostDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (ordercost == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOrdercost", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("ordermovId=");
		url.append(TagUtils.getInstance().filter(
				ordercost.getOrdermovId().toString()));
		url.append("&ordercostId=");
		url.append(TagUtils.getInstance().filter(
				ordercost.getOrdercostId().toString()));
