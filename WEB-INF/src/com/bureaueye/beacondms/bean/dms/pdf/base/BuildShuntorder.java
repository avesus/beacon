 ...
 * 	
 */
public final class BuildShuntorder
{
	...
	public void build(
		...
		try {
			...
			String title = resourceProps.getString("title.shuntorder");
	...
	public void doBody1() throws DocumentException, BadElementException,  Exception
	{
		...
		try {
			...
			label.setPhrase(new Phrase(": "+getTagValue("g4ldesc",elementLdgaddrkey)+", "+xmlToPdf.countryRule(elementLdgaddrkey),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);
		...
		try {
			...
			label.setPhrase(new Phrase(resourceProps.getString("prompt.deliveryplace"),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);
			label.setPhrase(new Phrase(": "+getTagValue("g4ldesc",elementDchaddrkey)+", "+xmlToPdf.countryRule(elementDchaddrkey),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);	
			
			label.setPhrase(new Phrase(resourceProps.getString("prompt.transportdatetime"),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);
			label.setPhrase(
					new Phrase(
		...
		try {
			float[] widths = {0.04f, 0.19f, 0.19f, 0.19f, 0.19f, 0.20f};
			...
			_text.addCell(new Phrase(resourceProps.getString("heading.containersize"),font));		
	...
	public void doBody2() throws DocumentException, BadElementException,  Exception
	{
		...
		try {
			...
			//derive notes
			String notes = "";	
			notes=resourceProps.getString("prompt.pleasemention");

			
			
			label.setPhrase(new Phrase(resourceProps.getString("prompt.extrainfo"),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
	...
	) throws DocumentException, BadElementException,  Exception
	{
		...
		try {
			...
			_text.addCell(new Phrase(getTagValue("lengthf",elementUnittec),font));	
