...
public class LinkCcyLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		Ccy currency = null;
		String name = "ccyLineItem";

		try {
			currency = (Ccy) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (currency == null)
			throw new JspException(messages.getMessage("lineItem.noCcy", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("ccykey=").append(
				TagUtils.getInstance().filter(currency.getCcykey()));
