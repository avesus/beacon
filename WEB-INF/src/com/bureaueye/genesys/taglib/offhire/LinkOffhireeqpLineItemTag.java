...
public class LinkOffhireeqpLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Offhireeqp lineItem = null;
		try {
			lineItem = (Offhireeqp) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOffhireeqp", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("offhireId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getOffhireId().toString()));
