 ...
 * ----------
 * 
 * LAKER BESPOKE (COPY FROM com.bureaueye.beacon.action.order.EditOrderhdrAction)
 *  
 */
public final class EditOrderhdrAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		if (action.equals(com.bureaueye.beacon.bean.Constants.CREATE_ACTION)) {
			...
			editForm.setOrderno("");
			editForm.setQuotno("");		
			editForm.setShipvessel2("");
			
			//only clear if not copied
			if (editForm.getCopyId().equals("")) {	
				editForm.setCustomeraddrkey("");
				editForm.setCustomerref("");			
				editForm.setLdglocationkey2("");
				editForm.setDchlocationkey2("");			
				editForm.setShipvessel("");
				editForm.setShipvendoraddrkey("");
				editForm.setShipvoyage("");			
				editForm.setNotes("");	
			}		
			...
			try {
				...
				//only default dates if not copied
				if (editForm.getCopyId().equals("")) {	
					editForm.setLdgfromdate(c.getTime());
					editForm.setLdgtodate(c.getTime());				
					editForm.setDchfromdate(c.getTime());				
					editForm.setDchtodate(c.getTime());		
				}
			} catch (Exception e) {}
