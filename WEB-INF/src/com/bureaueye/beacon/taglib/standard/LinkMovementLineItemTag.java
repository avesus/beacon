...
public class LinkMovementLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			movement = (MovementDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (movement == null)
			throw new JspException(messages.getMessage(
					"lineItem.noMovement", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				movement.getUnitId().toString()));
		url.append("&movementId=");
		url.append(TagUtils.getInstance().filter(
