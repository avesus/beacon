...
public class LinkCountryLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "countryLineItem";

		try {
			country = (Country) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (country == null)
			throw new JspException(messages.getMessage(
					"countryLineItem.noCountry", name));

		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("countrykey=");
		url.append(TagUtils.getInstance().filter(country.getCountrykey()));
