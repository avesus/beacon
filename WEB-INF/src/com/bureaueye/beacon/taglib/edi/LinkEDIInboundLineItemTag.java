package com.bureaueye.beacon.taglib.edi;
...
public class LinkEDIInboundLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Ediinbound lineItem = null;
		try {
			lineItem = (Ediinbound) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noEDIInbound", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("ediinboundId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getEdiinboundId().toString()));
