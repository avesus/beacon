...
public class LinkVendortariffhdrLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		VendortariffhdrDTO vendortariffhdr = null;
		String name = "vendortariffhdrLineItem";

		try {
			vendortariffhdr = (VendortariffhdrDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (vendortariffhdr == null)
			throw new JspException(messages.getMessage(
					"lineItem.noVendortariffhdr", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append(page.indexOf("action=") < 0 ? "id=" : "vendtarfhdrId=").append(
				vendortariffhdr.getVendtarfhdrId());
