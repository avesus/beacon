...
public class LinkContracteqpLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Contracteqp lineItem = null;
		try {
			lineItem = (Contracteqp) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noContracteqp", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
