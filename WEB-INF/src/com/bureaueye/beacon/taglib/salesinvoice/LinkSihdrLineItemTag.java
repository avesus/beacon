...
package com.bureaueye.beacon.taglib.salesinvoice;
...
public class LinkSihdrLineItemTag extends TagSupport {
     ...
     * The attribute name.
     */
    private String name = "sihdrLineItem";
	...
	url.append(config.getPrefix());
        url.append(page);
        SihdrDTO lineItem = null;
	try {
		lineItem = (SihdrDTO) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noSalesinvoiceLineItem", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getSihdrId().toString()));	
