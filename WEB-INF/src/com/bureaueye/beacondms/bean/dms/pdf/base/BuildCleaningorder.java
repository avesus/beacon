 ...
 * 	
 */
public final class BuildCleaningorder
{
	...
	public void build(
		...
		try {
			...
			String title=
				resourceProps.getString("title.cleaningorder");
	...
	public void doBody1() throws DocumentException, BadElementException,  Exception
	{
		...
		try {
			...
			label.setPhrase(new Phrase(resourceProps.getString("prompt.date"),fontBold));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);
			label.setPhrase(new Phrase(": "+Util.dateTextFormat(new Date()),fontBold));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);	
		...
		try {
			...
			label.setPhrase(new Phrase(resourceProps.getString("prompt.thefollowingtankswillbearriving"),font));
			label.setHorizontalAlignment(Element.ALIGN_LEFT);
			wrapper.addCell(label);
		...
		try {
			float[] widths = {0.04f, 0.17f, 0.11f, 0.28f, 0.28f, 0.12f};
			...
			_text.addCell(new Phrase(resourceProps.getString("heading.currentstatus"),font));	
			...
			_text.addCell(new Phrase(resourceProps.getString("heading.previousproduct"),font));		
			...
			_text.addCell(new Phrase(resourceProps.getString("heading.tankstatusavorpt"),font));	
	...
	) throws DocumentException, BadElementException,  Exception
	{
		...
		org.w3c.dom.Element elementPreviousproduct1 = null;
		
		try{elementUnitkey=(org.w3c.dom.Element) elementOrdhdr.getElementsByTagName("unitkey").item(0);}catch(Exception e){}
		try{elementPreviousproduct1=(org.w3c.dom.Element) elementOrdhdr.getElementsByTagName("previousproduct1").item(0);}catch(Exception e){}
		try{elementProdcode=(org.w3c.dom.Element) elementOrdhdr.getElementsByTagName("prodcode").item(0);}catch(Exception e){}
		...
		try {
			...
			_text.addCell(new Phrase(getTagValue("tradname",elementProdcode),font));	
			...
			_text.addCell(new Phrase(getTagValue("tradname",elementPreviousproduct1),font));	
	...
	public void doBody2() throws DocumentException, BadElementException,  Exception
	{
		...
		try {
			...
			label.setPhrase(new Phrase(resourceProps.getString("prompt.pleasebesure"),fontBold));
