...
public class LinkQuochargeLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "quochargeLineItem";

		QuochargeDTO quocharge = null;
		try {
			quocharge = (QuochargeDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (quocharge == null)
			throw new JspException(messages.getMessage(
					"lineItem.noQuocharge", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				quocharge.getQuohdrId().toString()));
		url.append("&quochargeId=");
		url.append(TagUtils.getInstance().filter(
				quocharge.getQuochargeId().toString()));
