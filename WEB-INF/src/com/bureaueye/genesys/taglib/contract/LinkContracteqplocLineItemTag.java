...
public class LinkContracteqplocLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		ContracteqplocDTO lineItem = null;
		try {
			lineItem = (ContracteqplocDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noContracteqploc", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("contracteqpId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getContracteqpId().toString()));
		url.append("&contracteqplocId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getContracteqplocId().toString()));
