...
public class LinkOrderhdrTag extends TagSupport {
     ...
     * The attribute name.
     */
    private String name = "OrderhdrForm";
	...
	url.append(config.getPrefix());
        ...
        OrderhdrForm form = null;
        try {
        	form = (OrderhdrForm) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	form = null;
        }     
	if (form == null)
	    throw new JspException
	        (messages.getMessage("form.noOrderhdr", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
