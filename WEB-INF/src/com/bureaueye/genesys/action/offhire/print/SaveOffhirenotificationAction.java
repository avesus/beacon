...
import com.bureaueye.genesys.action.offhire.pdf.EndPage;
...
/**
 */
public class SaveOffhirenotificationAction extends BaseAction {
	...
	public SaveOffhirenotificationAction() {
		try {
	...
	public void toPrint(PrinthdrForm saveForm, User user) throws ServletException, IOException
	{
		...
		_Table1_1.addCell(new Phrase("Off Hire Notification",fontHeading));
		...
		try {
			_Table1_10.addCell(
					new Phrase(
							"Please accept the following Unit(s) for off-hire on reference "+
		...
		// details header
		float[] widths1_2 = {0.2f, 0.3f, 0.3f, 0.2f};
		...
		_Table1_2.addCell(new Phrase("Delivery Date",fontSmallBold));
		...
		for (Iterator it = saveForm.getPrintdtls().iterator(); it.hasNext();) {
			...
			try {
				// details header
				float[] widths1_4 = {0.2f, 0.3f, 0.3f, 0.2f};
