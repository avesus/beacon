...
public class LinkContractLineItemTag extends TagSupport {
	...
	url.append(config.getPrefix());
        url.append(page);
        ContractDTO lineItem = null;
	try {
		lineItem = (ContractDTO) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noContractLineItem", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getContractId().toString()));	
