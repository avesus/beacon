...
public class LinkOffhireeqpotcLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		OffhireeqpotcDTO lineItem = null;
		try {
			lineItem = (OffhireeqpotcDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOffhireeqpotc", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("offhireeqpId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getOffhireeqpId().toString()));
		url.append("&offhireeqpotcId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getOffhireeqpotcId().toString()));
