...
public class LinkPodtlrcvdLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		PodtlrcvdDTO lineItem = null;
		try {
			lineItem = (PodtlrcvdDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noPodtlrcvdLineItem", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append("&podtlrcvdId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getPodtlrcvdId().toString()));
