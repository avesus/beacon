...
public class LinkJobcostLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "jobcostLineItem";

		JobcostDTO lineItem = null;
		try {
			lineItem = (JobcostDTO) pageContext.findAttribute(name);
		...
		url.append(TagUtils.getInstance().filter(
				lineItem.getJobmovId().toString()));
		url.append("&jobcostId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getJobcostId().toString()));
