package com.bureaueye.beacon.taglib.account;
 ...
 *								
 */
public class LinkAIOutLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Aiout lineItem = null;
		try {
			lineItem = (Aiout) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noAIOut", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
