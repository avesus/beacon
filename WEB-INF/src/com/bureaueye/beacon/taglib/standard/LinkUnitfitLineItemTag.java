...
public class LinkUnitfitLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "unitfitLineItem";

		UnitfitDTO lineItem = null;
		try {
			lineItem = (UnitfitDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noUnitfit", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				lineItem.getUnitId().toString()));
		url.append("&unitfitId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getUnitfitId().toString()));
