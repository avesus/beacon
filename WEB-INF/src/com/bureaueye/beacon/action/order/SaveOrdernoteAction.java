...
public final class SaveOrdernoteAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		OrdernoteForm ordernoteForm = (OrdernoteForm) form;
		String action = ordernoteForm.getAction();
		...
		OrdernoteBD bd = new OrdernoteBD(this.getSessionFactoryClusterMap());

		Integer pk = null;	
		try {
			pk = new Integer(ordernoteForm.getOrdernoteId());			
		} catch (Exception e) {}
		Ordernote dto = bd.read(pk);
		if (dto == null) {
			dto = new Ordernote();
		}		
		...
		BeanUtils.copyProperties(dto, ordernoteForm);

		
		if (action.equals("Delete")) {		
			bd.delete(new Ordernote(pk), user);		
		} else {
