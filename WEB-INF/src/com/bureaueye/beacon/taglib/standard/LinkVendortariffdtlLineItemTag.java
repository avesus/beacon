...
public class LinkVendortariffdtlLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "vendortariffdtlLineItem";

		try {
			vendortariffdtl = (Vendortariffdtl) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (vendortariffdtl == null)
			throw new JspException(messages.getMessage(
					"lineItem.noVendortariffdtl", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("vendtarfhdrId=").append(vendortariffdtl.getVendtarfhdrId());
		url.append("&vendtarfdtlId=")
				.append(vendortariffdtl.getVendtarfdtlId());
