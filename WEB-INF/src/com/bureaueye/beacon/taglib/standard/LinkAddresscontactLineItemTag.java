...
public class LinkAddresscontactLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "addresscontactLineItem";

		Addresscontact addresscontact = null;
		try {
			addresscontact = (Addresscontact) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (addresscontact == null)
			throw new JspException(messages.getMessage(
					"lineItem.noAddresscontact", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				addresscontact.getId().getAddrkey()));
		url.append("&contactkey=");
		url.append(TagUtils.getInstance().filter(
				addresscontact.getId().getContactkey()));
