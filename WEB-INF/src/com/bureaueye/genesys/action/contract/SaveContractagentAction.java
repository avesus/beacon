...
public final class SaveContractagentAction extends BaseAction {
	...
	throws Exception {
		...
		ContractagentForm saveForm = (ContractagentForm) form;
		...
		ContractagentBD bd = new ContractagentBD(this.getSessionFactoryClusterMap());
		
		// set primary key
		Integer pk = null;		
		try {
			pk = new Integer(saveForm.getContractagentId());			
		} catch (Exception e) {}
		// find data-access-object using primary key
		Contractagent dto = bd.read(pk);
		// not found create empty object
		if (dto == null) {
			dto = new Contractagent();
		}
