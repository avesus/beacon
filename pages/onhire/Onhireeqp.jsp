...
function unitSearch(field, status, deleted, inactive, blockalloc){
	var url     = 'EditUnitSearchOnhire.do?maxResults=999'+
					'&headerInfo1='+status+
					'&headerInfo2='+field+
					'&headerInfo3='+deleted+
					'&headerInfo4='+inactive+
					'&headerInfo5='+blockalloc+
					'&headerInfo6='+document.forms[0].elements['onhireId'].value+
...
<logic:equal name="OnhireeqpForm" property="action"
...
<span class="header2">&gt;</span>                         
      	  		<html:link action="/ListOnhireeqp"><span class="header4"><bean:message key="onhireeqp.title.list"/></span></html:link>               
<span class="header2">&gt;</span>                         
...
<html:form action="/SaveOnhireeqp">
	
<html:hidden property="onhireeqpId"/>
<html:hidden property="onhireId"/>
<html:hidden property="action"/>
<html:hidden property="copyId"/>
<html:hidden property="unitId"/>
<html:hidden property="onhiretimehh" value="00"/>
<html:hidden property="onhiretimemm" value="00"/>
<html:hidden property="onhiretimess" value="00"/>
...
<logic:notEqual name="OnhireeqpForm" property="action"
                     scope="request" value="Delete">  
...
<tr>
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="1" class="onhireBgcolor">  				
        <tr>
          ...
          <TD class="td9" width="20%" align="right"><bean:message key="prompt.unit"/>:</TD>
          <TD class="td9" width="80%">
		  <c:if test="${OnhireeqpForm.action == 'Create'}">
			<html:text property="unitkey" size="20" readonly="true" maxlength="20" styleClass="ftforminputsmall"/>
			<html:link href="javascript:void(unitSearch('unitkey','AVAILABLE','N','N','N'))">
		...
		<html:select property="reservedateyyyy" styleClass="ftforminputsmall">
		  ...
		  <c:if test="${OnhireeqpForm.action != 'Create'}">  
			<bean:write name="OnhireeqpForm" property="reservedate" format="dd-MMM-yyyy" filter="true"/>
        ...
        <tr>
          <td width="20%" class="td9">
		  <div align="right">
		       <bean:message key="prompt.onhiredate"/>:
		  </div>		  
		  </td>
          <td width="80%" class="td9">	
		  <c:if test="${OnhireeqpForm.actualflag == 'false'}">  		  	  
		<html:select property="onhiredatedd" styleClass="ftforminputsmall">
			<html:option value=""><bean:message key="prompt.day"/></html:option>				
			<html:options collection="days" property="value" labelProperty="label"/>	
		</html:select>		  
		<html:select property="onhiredatemm" styleClass="ftforminputsmall">
			<html:option value=""><bean:message key="prompt.month"/></html:option>				
			<html:options collection="months" property="value" labelProperty="label"/>	
		</html:select>
		<html:select property="onhiredateyyyy" styleClass="ftforminputsmall">
			<html:option value=""><bean:message key="prompt.year"/></html:option>				
			<html:options collection="years" property="value" labelProperty="label"/>	
		</html:select>&nbsp;<html:checkbox property="actualflag" />&nbsp;<bean:message key="prompt.actual"/>	
		  </c:if>
		  <c:if test="${OnhireeqpForm.actualflag != 'false'}">  
			<bean:write name="OnhireeqpForm" property="onhiredate" format="dd-MMM-yyyy" filter="true"/>
			<html:hidden property="onhiredatemm"/>
			<html:hidden property="onhiredatedd"/>
			<html:hidden property="onhiredateyyyy"/>
			<html:hidden property="actualflag"/>
			&nbsp;<img src="images/icon_tick_12_12.gif" width="12" height="12"/>&nbsp;<bean:message key="prompt.actual"/>		  </c:if>					  		 	  
          </td>
        </tr>
...
<tr>
			        <td colspan="2">
				        ...
				        <div class="btnPane"> 
				            <logic:equal name="OnhireeqpForm" property="action"   scope="request" value="Create">  
				                <button id="buttonStore" class="btn btnFF" type="submit">
                				    <img class=" IM tick-IM" title="" src="images/buttons/clear1x1.gif" alt="" style="border-width:0px;" />
				                    <span><bean:message key="button.add"/></span>
                				</button>      
				            </logic:equal>
          					<logic:equal name="OnhireeqpForm" property="action"   scope="request" value="Delete">
							...
							<logic:equal name="OnhireeqpForm" property="action"   scope="request" value="Edit">
							...
							<logic:notEqual name="OnhireeqpForm" property="action"   scope="request" value="View">                            
							...
							<logic:equal name="OnhireeqpForm" property="action"   scope="request" value="View">                            
								<button id="exitButton" class="btn btnFF" type="button"  onclick="document.location.href='ListOnhireeqp.do'">
			                    	<img class=" IM cross-IM" title="" src="images/buttons/clear1x1.gif" alt="" style="border-width:0px;" />
