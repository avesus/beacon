package com.bureaueye.beacon.action.account.mas500.laker;
 ...
 * Amendments ----------
 * 
 * NT 2014-01-21 WSI-201401-0001 BEACON LAKER (WSI LINER) - MAS500 AR EXTRACT
 * 
 * NT 2014-02-17 WSI-201401-0001 BEACON LAKER (WSI LINER) - ADD MAS500 AP EXTRACT
 * 
...
public final class BuildAIOutAction {
	...
	public static final String COMPANY_ID = "WSI";
	...
	public TreeMap<String, StringBuffer> build(Aiout aioutdao,
		...
		try {
			if (aioutdao.getDoctype().equals(
					com.bureaueye.beacon.bean.Constants.AR_ACTUAL)) {
				returnParams = buildAR_ACTUAL(aioutdao);
			}
		} catch (Exception e) {
			log.error("[" + this.getClass().getName() + "] " + new Date()
					+ " Build Document: " + aioutdao.getDoctype() + " Dockey: "
					+ aioutdao.getDockey() + " Exception: " + e.getMessage());
		}
	...
	public static TreeMap<String, StringBuffer> buildAR_ACTUAL(Aiout aioutdao)
			throws Exception {
		...
		try {
			sidtldaos = sidtlbd.findSidtlsById(
					sihdrdao.getSihdrId().toString(), 0,
		...
		if (sihdrdao == null) {
			errors.append(setError(
					"0001",
					"sihdr",
					"Sales Invoice Header not found for ["
							+ aioutdao.getDockey() + "]"));
		}
		...
		if (accSyscompid.equals("")) {
			errors.append(setError(
					"0002",
					"company",
					"Company Mapping code Not Found for ["
		...
		if (sidtldaos.size() == 0) {
			errors.append(setError(
					"0003",
					"sidtl",
					"Sales Invoice Details Not Found for ["
							+ aioutdao.getDockey() + "]"));
		}

		// check order no exists
		if (sihdrdao.getOrderno().equals("")) {
			warnings.append(setWarning("0005", "sihdr",
		...
		try {
			...
			try {
				...
				warnings.append(setWarning(
						"0010",
						"address",
						"Address not found for Code ["
								+ sihdrdao.getCustomeraddrkey() + "]"));
			}

			document.append(headerPrefix);

			try {
				document.append(setHeader("CompanyId", COMPANY_ID));
				segmentCount++;
			} catch (Exception e) {
			}
			...
			try {
				document.append(setHeader("InvCurrID",
						sihdrdao.getInvoiceccykey()));
				segmentCount++;
			} catch (Exception e) {
			}
			...
			try {
				document.append(setHeader("Tranamt", sihdrdao.getInvoiceamt()
						.toString()));
				segmentCount++;
			} catch (Exception e) {
			}
			...
			try {
				document.append(setHeader("TranDate",
						com.bureaueye.beacon.util.Util.dateTextFormat6(sihdrdao
								.getPrintdate())));
				segmentCount++;
			} catch (Exception e) {
			}
...
/*			try {
			...
			try {
				document.append(setHeader("WSContainerID", sihdrdao.getAdddata1()));
				segmentCount++;
			} catch (Exception e) {
			}

			try {
				document.append(setHeader("WSBOL", ""));
				segmentCount++;
			} catch (Exception e) {
			}
		...
		while (it.hasNext()) {
			...
			try {
				...
				try {
					document.append(setDetail(recordno, "Description",
							sidtldao.getTxt1()));
					segmentCount++;
				} catch (Exception e) {
				}
				...
				try {
					document.append(setDetail(recordno, "ExtAmt", sidtldao
							.getChgamt().toString()));
					segmentCount++;
				} catch (Exception e) {
				}
				...
				try {
					document.append(setDetail(recordno, "ItemID", sidtldao
							.getChargekey().getAccsysitemid()));
					segmentCount++;
				} catch (Exception e) {
				}

				try {
					document.append(setDetail(recordno, "QtyShipped", sidtldao
							.getUnits().toString()));
					segmentCount++;
				} catch (Exception e) {
				}
				...
				try {
					document.append(setDetail(recordno, "GLAcctNo", sidtldao
							.getChargekey().getIncacct()));
					segmentCount++;
				} catch (Exception e) {
				}
				...
				try {
					document.append(setDetail(recordno, "UnitPrice", sidtldao
							.getRate().toString()));
					segmentCount++;
				} catch (Exception e) {
				}
...
/*			try {
	...
	public static String setWarning(String id, String field, String msg) {
		String _warning = "";
		_warning = warningPrefix + idPrefix + "<![CDATA[" + id + "]]>"
				+ idSuffix + fieldPrefix + "<![CDATA[" + field + "]]>"
				+ fieldSuffix + msgPrefix + "<![CDATA[" + msg + "]]>"
				+ msgSuffix + warningSuffix;
	...
	public static String setError(String id, String field, String msg) {
		String _error = "";
		_error = errorPrefix + idPrefix + "<![CDATA[" + id + "]]>" + idSuffix
				+ fieldPrefix + "<![CDATA[" + field + "]]>" + fieldSuffix
				+ msgPrefix + "<![CDATA[" + msg + "]]>" + msgSuffix
				+ errorSuffix;
	...
	public static String setHeader(String name, String value) {
		 ...
		 * returnString=returnString+namePrefix
		 * +"<![CDATA["+name+"]]>"+nameSuffix;
		 * returnString=returnString+valuePrefix
		 * +"<![CDATA["+value+"]]>"+valueSuffix;
		 * returnString=returnString+headerSuffix;
		 */
		String returnString = "<" + name + ">" + "<![CDATA[" + value + "]]>"
				+ "</" + name + ">";
		return returnString;
	}
	...
	public static String setDetail(double recordno, String name, String value) {
		 ...
		 * returnString=returnString+recordnoPrefix
		 * +"<![CDATA["+recordno+"]]>"+recordnoSuffix;
		 * returnString=returnString+
		 * namePrefix+"<![CDATA["+name+"]]>"+nameSuffix;
		 * returnString=returnString
		 * +valuePrefix+"<![CDATA["+value+"]]>"+valueSuffix;
