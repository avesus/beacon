...
public final class RefreshOrderCcyexServlet extends BaseAJAX {
	...
	throws ServletException, IOException {
		...
		if (id1.equals("0") || id1.equals("") || id1==null) {
			try {
				id1 = new OrdermovBD(this.getSessionFactoryClusterMap()).read(new Integer(id2)).getOrderhdrId().toString();
			} catch (Exception e) {				
			}
		}		
		// find order exchange rate date
		try {
			exchgdate = new OrderhdrBD(this.getSessionFactoryClusterMap()).read(new Integer(id1)).getExchgdate();
		} catch (Exception e) {				
		}		
