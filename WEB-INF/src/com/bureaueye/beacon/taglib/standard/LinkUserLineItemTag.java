...
public class LinkUserLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		User lineItem = null;
		String name = "userLineItem";

		try {
			lineItem = (User) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage("lineItem.noUser",
					name));
		...
		if (page.indexOf("?") < 0)
			...
			url.append("&");
		url.append(page.indexOf("action=") < 0 ? "id=" : "userid=").append(
				TagUtils.getInstance().filter(lineItem.getUserid()));
		//url.append("id=");
		//url.append(TagUtils.getInstance().filter(lineItem.getUserid()));
