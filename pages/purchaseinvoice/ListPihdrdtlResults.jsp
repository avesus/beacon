...
<table border="0" cellpadding="0" cellspacing="0" width="60%" bgcolor="#ffffff"> 
	...
	<tr><td>			  
					<div class="btnPane3">	              
						<button id="addButton" class="btn btnFF" type="button" onClick="document.location.href='EditPihdrdtl.do?action=Create'">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>   
		<td class="column-header1" width="10%">              
                    				<button id="sortButton" class="column-header-btn1" type="button" onClick="document.location.href='ListPihdrdtl.do?orderBy=Pino&gotoPage=0'+'&amp;orderByDesc='+orderByDesc.options[orderByDesc.selectedIndex].value">
										<span><bean:message key="heading.pino"/></span>                        
										<logic:notEqual name="ListPihdrdtlForm" property="orderBy" scope="session" value="Pino">              
									         <img class=" IM2 Spacer-IM2" title="" src="images/buttons/clear1x1.gif" alt="" style="border-width:0px;" />       
										</logic:notEqual>
										<logic:equal name="ListPihdrdtlForm" property="orderBy" scope="session" value="Pino">              
		...
		<td class="column-header2" width="10%">              
                    				<button id="sortButton" class="column-header-btn1" type="button" onClick="document.location.href='ListPihdrdtl.do?orderBy=Invoicedate&gotoPage=0'+'&amp;orderByDesc='+orderByDesc.options[orderByDesc.selectedIndex].value">
										<span><bean:message key="heading.invoicedate"/></span>                        
										<logic:notEqual name="ListPihdrdtlForm" property="orderBy" scope="session" value="Invoicedate">              
									         <img class=" IM2 Spacer-IM2" title="" src="images/buttons/clear1x1.gif" alt="" style="border-width:0px;" />       
										</logic:notEqual>
										<logic:equal name="ListPihdrdtlForm" property="orderBy" scope="session" value="Invoicedate">              
									         <img class=" IM2 Sortdown-IM2" title="" src="images/buttons/clear1x1.gif" alt="" style="border-width:0px;" />       
										</logic:equal>
							        </button>
		</td>   
		...
		<td class="column-header2" width="10%" align="right">
										<span><bean:message key="heading.netamount"/></span>                            
        </td>  
		<td class="column-header2" width="5%">
										<span><bean:message key="heading.approved"/></span>                             
        </td>
		<td class="column-header2" width="5%">
										<span><bean:message key="heading.miscellaneous"/></span>                             
        </td>          
...
<logic:notEmpty name="ListPihdrdtlForm" property="lineItems">

<%int row=0;%>

          <logic:iterate id="lineItem" name="ListPihdrdtlForm" property="lineItems">
...
<bean:write name="lineItem" property="netamt" filter="true"/>&nbsp;         
            </td>                        
			...
			<td class="<%= _class2 %>" nowrap align="center">
                      <c:if test="${lineItem.miscflag == 'true'}">
					  	<img src="images/icon_tick_12_12.gif" width="12" height="12" alt='<c:out value="${lineItem.invoicestatus}"/>'/>
                      </c:if>            
            </td> 
...
<html:link action="/EditPihdrdtl.do?action=Edit" paramId="id" paramName="lineItem" paramProperty="pihdrId"><bean:message key="prompt.edit"/></html:link>	
</c:if>	

<c:if test="${lineItem.invoicestatus=='DISPUTED'}">                          
<span class="label5">&nbsp;&nbsp;|&nbsp;&nbsp;</span>  
<html:link action="/EditPihdrdtl.do?action=Delete" paramId="id" paramName="lineItem" paramProperty="pihdrId">
<bean:message key="prompt.delete"/> 
</html:link>	
</c:if>	

<c:if test="${lineItem.postedflag==true}">	
<html:link action="/EditPihdrdtl.do?action=View" paramId="id" paramName="lineItem" paramProperty="pihdrId"><bean:message key="prompt.view"/></html:link>		
</c:if>	
...
<!-- row2 -->
                  <tr>
                    <td class="<%= _class %>" colspan="12">
<html:link action="/ListPihdrcost.do" paramId="id" paramName="lineItem" paramProperty="pidtlId"><bean:message key="prompt.allocatecosts"/></html:link>                    
                    </td>
                  </tr>
