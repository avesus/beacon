...
public class LinkPohdrLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		PohdrDTO lineItem = null;
		try {
			lineItem = (PohdrDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
			lineItem = null;
		}
		if (lineItem == null)
			throw new JspException
			(messages.getMessage("lineItem.noPohdrLineItem", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("id=");
		url.append(TagUtils.getInstance().filter(lineItem.getPohdrId().toString()));	
