package com.bureaueye.genesys.taglib.contract;
...
public class LinkContractagentLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		ContractagentDTO lineItem = null;
		try {
			lineItem = (ContractagentDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noContractagent", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("contractId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getContractId().toString()));
		url.append("&contractagentId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getContractagentId().toString()));
