...
public final class SaveSectiontemplateAction extends BaseAction {
	...
	throws Exception {
		...
		SectiontemplateForm saveForm = (SectiontemplateForm) form;
		...
		Integer pk = null;		
		try {
			pk = new Integer(saveForm.getSectiontemplateId());			
		} catch (Exception e) {}
		// find data-access-object using primary key
		Sectiontemplate dao = bd.read(pk);
		// not found create empty object
		if (dao == null) {
			dao = new Sectiontemplate();
		}
		...
		if (action.equals("Delete")) {		
			bd.delete(new Sectiontemplate(pk),user);		
		} else {
