...
public class LinkQuomovcostLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "quomovcostLineItem";

		QuocostDTO quocost = null;
		try {
			quocost = (QuocostDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (quocost == null)
			throw new JspException(messages.getMessage(
					"lineItem.noQuocost", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				quocost.getQuomovId().toString()));
		url.append("&quocostId=");
		url.append(TagUtils.getInstance().filter(
				quocost.getQuocostId().toString()));
