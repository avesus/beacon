...
public class LinkProductLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "productLineItem";

		try {
			lineItem = (Product) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage("lineItem.noProduct",
					name));
		...
		if (page.indexOf("?") < 0)
			...
			url.append("&");
		url.append("productkey=").append(
				TagUtils.getInstance().filter(lineItem.getProductkey()));
