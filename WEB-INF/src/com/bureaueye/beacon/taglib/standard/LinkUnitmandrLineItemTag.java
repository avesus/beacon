...
public class LinkUnitmandrLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		UnitmandrDTO lineItem = null;
		try {
			lineItem = (UnitmandrDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noUnitmandr", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append("&unitmandrId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getUnitmandrId().toString()));
