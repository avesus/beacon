...
public class LinkUsergroupmenuLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "usergroupmenuLineItem";

		Usergroupmenu lineItem = null;
		try {
			lineItem = (Usergroupmenu) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noUsergroupmenu", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		url.append("usergroupid=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getUsergroupid()));
		url.append("&usergroupmenuId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getUsergroupmenuId().toString()));
