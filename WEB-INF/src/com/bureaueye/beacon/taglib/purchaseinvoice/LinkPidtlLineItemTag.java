...
public class LinkPidtlLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		PidtlDTO lineItem = null;
		try {
			lineItem = (PidtlDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noPidtlLineItem", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				lineItem.getPihdrId().toString()));
		url.append("&pidtlId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getPidtlId().toString()));
