...
import com.bureaueye.beacon.form.quotation.WorkflowLineItem;
...
public class LinkWorkflowLineItemTag extends TagSupport {
	...
	url.append(config.getPrefix());
        url.append(page);
        WorkflowLineItem lineItem = null;
	try {
		lineItem = (WorkflowLineItem) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noQuohdr", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getQuohdrId().toString()));	
