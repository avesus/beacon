 ...
 *
 */
public final class EditQuonoteAction extends BaseAction {
	...
	throws Exception {
		...
		QuonoteForm editForm = (QuonoteForm) form;
		...
		if (!(action == null || action.equals("Create"))) {
			...
			Quonote dto = new QuonoteBD(this.getSessionFactoryClusterMap())
			.read(new Integer(editForm.getQuonoteId()));
			
			if (dto == null) {
				log.error("["+this.getClass().getName()+"] "+new java.util.Date()+" No Quonote found for orderhdrId: "
						+ editForm.getQuohdrId() + ", "
						+ "orderchargeid: " + editForm.getQuonoteId());
				ActionMessages errors = new ActionMessages();
				errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
						"error.record.deleted", editForm.getQuohdrId()
						+ " - " + editForm.getQuonoteId()));
