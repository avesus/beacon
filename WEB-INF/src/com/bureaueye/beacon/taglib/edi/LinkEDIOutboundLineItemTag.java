...
public class LinkEDIOutboundLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Edioutbound lineItem = null;
		try {
			lineItem = (Edioutbound) pageContext.findAttribute(name);
		...
		if (page.indexOf("?") < 0)
			...
			url.append("&");
		url.append("edioutboundId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getEdioutboundId().toString()));
