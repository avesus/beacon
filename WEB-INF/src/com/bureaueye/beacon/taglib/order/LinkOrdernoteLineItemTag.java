...
public class LinkOrdernoteLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "ordernoteLineItem";

		Ordernote ordernote = null;
		try {
			ordernote = (Ordernote) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (ordernote == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOrdernote", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				ordernote.getOrderhdrId().toString()));
		url.append("&ordernoteId=");
		url.append(TagUtils.getInstance().filter(
				ordernote.getOrdernoteId().toString()));
