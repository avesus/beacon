...
package com.bureaueye.genesys.taglib.finance;
...
public class LinkBillingviewLineItemTag extends TagSupport {
	...
	url.append(config.getPrefix());
        url.append(page);
        FinancehdrDTO lineItem = null;
	try {
		lineItem = (FinancehdrDTO) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noBillingviewLineItem", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getFinancehdrId().toString()));	
