...
public final class SaveCostAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		if (action.equals("Delete")) {
			costbd.delete(new Cost(costform.getCostkey()), user);
		} else {
			Cost cost = costbd.read(costform.getCostkey());

			if (cost == null) {
				cost = new Cost();
			}

			BeanUtils.copyProperties(cost, costform);

			costbd.createOrUpdate(cost, user);

			// Find the ListCostForm and reset to first page
			...
			//listform.setSearchString1(cost.getCostkey());
