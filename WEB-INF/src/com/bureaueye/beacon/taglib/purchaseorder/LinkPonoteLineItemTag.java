...
public class LinkPonoteLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Ponote lineItem = null;
		try {
			lineItem = (Ponote) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (lineItem == null)
			throw new JspException(messages.getMessage(
					"lineItem.noPonote", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				lineItem.getPohdrId().toString()));
		url.append("&ponoteId=");
		url.append(TagUtils.getInstance().filter(
				lineItem.getPonoteId().toString()));
