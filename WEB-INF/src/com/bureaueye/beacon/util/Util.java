...
public class Util {
	...
	public static boolean invalidChars(String str) {
		...
		for (int i = 0; i < stringSize; i++) {
			if(	!(
					...
					Constants.VALID_INPUT_CHARS.indexOf(str.charAt(i)) > -1
			) 
			) return true;
		}
		return false;
	...
	/*
	 public static String secsToTime(String seconds) {
	 int secondsRemainder = 0;
	 try {
	 secondsRemainder = new Integer(seconds).intValue();
	 ...
	 secondsRemainder = secondsRemainder % 3600;
	 int minutes = secondsRemainder / 60;
	 secondsRemainder = secondsRemainder % 60;
	 return (nf.format(hours)+":"+nf.format(minutes)+":"+nf.format(secondsRemainder));
	 } catch (Exception e) {
	 return "00:00:00";
	 }
	 int seconds2 = 0;
	 try {
	 seconds2 = new Integer(seconds).intValue();
	 } catch (NumberFormatException npe) {
	 return "00:00:00";
	 }
	 int millisecond = seconds2*1000;
	...
	/**
	 * Assigns a globally unique Control Id
	 */
	public static synchronized String getNextControlId(String file)
	{
		String key = "";
		try {
			...
			// Increment and rewrite the file
			int suffix = Integer.parseInt(key);
			suffix++;
			String newKey = String.valueOf(suffix);
