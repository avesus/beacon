...
public class LinkSidtlLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "sidtlLineItem";

		SidtlDTO sidtl = null;
		try {
			sidtl = (SidtlDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (sidtl == null)
			throw new JspException(messages.getMessage(
					"lineItem.noSidtl", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				sidtl.getSihdrId().toString()));
		url.append("&sidtlId=");
		url.append(TagUtils.getInstance().filter(
				sidtl.getSidtlId().toString()));
