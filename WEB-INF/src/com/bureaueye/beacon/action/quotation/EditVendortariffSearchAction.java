...
public final class EditVendortariffSearchAction extends PopupBaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		log.debug("["+this.getClass().getName()+"] "+new java.util.Date()+" EditVendortariffSearchAction: " + vendortariffsearchForm.getId());

		Vendortariffhdr vendortariffhdr = new VendortariffhdrBD(this.getSessionFactoryClusterMap()).read(vendortariffsearchForm.getId());

		if (vendortariffhdr == null) {
			log.debug("["+this.getClass().getName()+"] "+new java.util.Date()+" No Vendor Tariff Header found for ID "
					+ vendortariffsearchForm.getId());
			
			// initialise parameters
			Quomov mov = null;
			Quohdr hdr = null;
			Quoprod quoprod = null;				
			...
			try {mov = movbd.read(new Integer(vendortariffsearchForm.getSearchString8()));} catch (Exception e) {}
			if (mov != null) {
				hdr = hdrbd.read(mov.getQuohdrId());				
				vendortariffsearchForm.setSearchString3(mov.getFromlocationkey());
				vendortariffsearchForm.setSearchString4(mov.getTolocationkey());
			} else {
				try {hdr = hdrbd.read(new Integer(vendortariffsearchForm.getSearchString11()));} catch (Exception e) {}
			}	
			...
			if (hdr != null) {
				...
				try {
					quoprod = quoprodbd.findFirstQuoprodByQuohdrId(hdr.getId());
					product = productbd.read(quoprod.getProductkey());
					if (product.isHazardous()) vendortariffsearchForm.setSearchString12("Y");
					if (!product.isHazardous()) vendortariffsearchForm.setSearchString12("N");					
				} catch (Exception e) {}
			}
			...
			log.debug("["+this.getClass().getName()+"] "+new java.util.Date()+" Populating Form from " + vendortariffhdr);
			vendortariffsearchForm.setStartPage();
			vendortariffsearchForm.setSearchString1(vendortariffhdr.getVendoraddrkey());
			vendortariffsearchForm.setSearchString2(vendortariffhdr.getCostkey());
			...
			vendortariffsearchForm.setSearchString7(vendortariffhdr.getCcykey());			
