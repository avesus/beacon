...
import com.bureaueye.beacon.form.standard.UsergroupmenuForm;
...
public final class EditUsergroupmenuAction extends BaseAction {

	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {

		UsergroupmenuForm editForm = (UsergroupmenuForm) form;
		...
		if (!(action == null || action.equals("Create"))) {
			...
			Integer pk = new Integer(editForm.getUsergroupmenuId());
			Usergroupmenu dto = new UsergroupmenuBD(this.getSessionFactoryClusterMap()).read(pk);

			if (dto == null) {
				log.error("["+this.getClass().getName()+"] "+new java.util.Date()+
						" No Record found for Id: "+ editForm.getUsergroupmenuId());
				ActionMessages errors = new ActionMessages();
				errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
						"error.record.deleted", editForm.getUsergroupmenuId()));
