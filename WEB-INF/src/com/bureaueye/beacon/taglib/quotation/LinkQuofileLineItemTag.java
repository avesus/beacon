...
public class LinkQuofileLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "quofileLineItem";

		Quofile quofile = null;
		try {
			quofile = (Quofile) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (quofile == null)
			throw new JspException(messages.getMessage(
					"lineItem.noQuofile", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				quofile.getQuohdrId().toString()));
		url.append("&quofileId=");
		url.append(TagUtils.getInstance().filter(
				quofile.getQuofileId().toString()));
