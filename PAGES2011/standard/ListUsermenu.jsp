...
<%--      <td height="20" bgcolor="#CCCCCC">&nbsp;</td>--%>  
	<td class="tdwht"><span class="heading2">     
		...
		<span class="heading2">
			<html:link action="/ListUsermenu" styleClass="link5">
...
<table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
  ...
  <tr class="text2"> 
    <td class="td6">
	&nbsp;<bean:write name="ListUsermenuForm" property="headerInfo1" scope="session"/>
	</td>							
   </tr> 
...
<table border="0" cellpadding="2" cellspacing="2" width="100%" bgcolor="#ffffff">
  <tr>
	<td align="right">
<logic:equal name="ListUsermenuForm" property="firstPage"
			scope="session" value="false">			
<INPUT type="image" id="startButton" name="startButton" src="images/arrow_start.gif"/>
 </logic:equal>	
<logic:notEqual name="ListUsermenuForm" property="firstPage"
			scope="session" value="false">
...
<logic:equal name="ListUsermenuForm" property="lastPage"
			scope="session" value="false">
<INPUT type="image" id="nextButton" name="nextButton" src="images/arrow_next.gif"/>				
</logic:equal>
<logic:notEqual name="ListUsermenuForm" property="lastPage"
			scope="session" value="false">
...
<app:linkUsermenuCreate page="/EditUsermenu.do?action=Create">
	<bean:message key="prompt.addamenu"/>
</app:linkUsermenuCreate>											
						</td>
					</tr>
					</table>									
					</td>
				</tr>		
			<c:forEach var="usermenuLineItem" items="${ListUsermenuForm.lineItems}" varStatus="loop">
				...
				<span class="text2">			
					<td class='<c:out value="${_class}"/>'>
						<c:out value="${usermenuLineItem.menuid}"/>
					</td> 
					<td class='<c:out value="${_class}"/>'>
						<c:out value="${usermenuLineItem.programid}"/>					
					</td> 
					<td class='<c:out value="${_class}"/>'>
						<c:out value="${usermenuLineItem.programtype}"/>
					</td> 	
					<td class='<c:out value="${_class}"/>'>
						<c:out value="${usermenuLineItem.sequence}"/>
					</td> 															 						
