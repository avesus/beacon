...
public class LinkOnhireLineItemTag extends TagSupport {
	...
	url.append(config.getPrefix());
        url.append(page);
        OnhireDTO lineItem = null;
	try {
		lineItem = (OnhireDTO) pageContext.findAttribute(name);
        } catch (ClassCastException e) {
        	lineItem = null;
	}
	if (lineItem == null)
	    throw new JspException
	        (messages.getMessage("lineItem.noOnhireLineItem", name));
	if (page.indexOf("?") < 0)
	    url.append("?");
	else
	    url.append("&");
	url.append("id=");
	url.append(TagUtils.getInstance().filter(lineItem.getOnhireId().toString()));	
