...
public class LinkVesselLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		url.append(page);
		Vessel vessel = null;
		String name = "vesselLineItem";

		try {
			vessel = (Vessel) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (vessel == null)
			throw new JspException(messages.getMessage("lineItem.noVessel",
					name));
		...
		if (page.indexOf("?") < 0)
			...
			url.append("&");
		url.append("vesselkey=");
		url.append(TagUtils.getInstance().filter(vessel.getVesselkey()));
