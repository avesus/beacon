...
public class LinkOrderprodLineItemTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String name = "orderprodLineItem";

		OrderprodDTO orderprod = null;
		try {
			orderprod = (OrderprodDTO) pageContext.findAttribute(name);
		} catch (ClassCastException e) {
		}
		if (orderprod == null)
			throw new JspException(messages.getMessage(
					"lineItem.noOrderprod", name));
		if (page.indexOf("?") < 0)
			url.append("?");
		else
			url.append("&");
		...
		url.append(TagUtils.getInstance().filter(
				orderprod.getOrderhdrId().toString()));
		url.append("&orderprodId=");
		url.append(TagUtils.getInstance().filter(
				orderprod.getOrderprodId().toString()));
